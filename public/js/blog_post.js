$(document).ready(function(){
    $('.head .logo a').text("<< SivaGao's Blog");
    ...
    var menuIndex = function(){
        ...
        if($('h2', '.content').length > 2 && !isMobile.any() && !ie6 ){ //,$('#content'))
            ...
            $('.sidebar').replaceWith('<div id="menuIndex"></div>');
            ...
            $(window).load(function(){
                ...
                $(window).scroll(function(){ 
                    ...
                    // need fix when you scroll at footer!
                    if ( (nowTop + $('#menuIndex').height()) > $('.footer').offset().top ) {
                        $('#menuIndex').fadeOut();
                    } else {
                        $('#menuIndex:hidden').fadeIn();
                    }
            ...
            //用js计算屏幕的高度
            $('#menuIndex').css('max-height',$(window).height()-80).addClass('sidebar');
        }
    };
    menuIndex();
